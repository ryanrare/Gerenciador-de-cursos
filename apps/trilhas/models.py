...
class Trilha(db.Model):
    ...
    cursos = db.relationship('Curso', secondary=trilha_curso, lazy='subquery',
                             backref=db.backref('trilhas', lazy=True))
    aulas = db.relationship('Aula', secondary=trilha_aula, lazy='subquery',
                            backref=db.backref('trilhas', lazy=True))
    comentarios = db.relationship('Comentario', secondary=trilha_comentario, lazy='subquery',
                                  backref=db.backref('trilhas', lazy=True))
    users = db.relationship('User', secondary=trilha_user, lazy='subquery',
                            backref=db.backref('trilhas', lazy=True))
